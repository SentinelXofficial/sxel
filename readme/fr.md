...
sxel -u "http://testphp.vulnweb.com/listproducts.php?cat=1"
sxel -u "http://target.com" --deep --all --prove --delve --vault --rank
sxel -u "http://target.com" --all --sarif results.sarif --ci
sxel --strike results.json
sxel --bundle results.json --bundle-out ./submission/
sxel --diff yesterday.json today.json
sxel -u "http://target.com" --all --hook https://hooks.slack.com/xxx
```
...
```bash
git clone https://github.com/SentinelXofficial/sxel
cd sxel && go mod tidy && go build -o sxel ./cmd/sxel
```
